...
public class OrderItem extends BaseEntity{
    ...
    private Item item;
    //  하나의 상품은 여러 주문 상품으로 들어갈 수 있으므로 주문 상품 기준으로 다대일 단방향 매핑으로 설정
    ...
    public static OrderItem createOrderItem(Item item, int count){
        ...
        orderItem.setCount(count); // 상품의 수량을 세팅
        orderItem.setOrderPrice(item.getPrice()); //현재 시간 기준으로 상품 가격을 주문 가격으로 세팅(실제 쇼핑몰 운영에서는 할인에 따라 변동가능)
